...
DygraphCanvasRenderer._fastCanvasProxy = function (context) {
  ...
  var addAction = function addAction(action, x, y) {
    ...
    if (lastRoundedX === null || rx != lastRoundedX) {
      ...
      var hasGapOnright = lastRoundedX - lastFlushedX > 1,
          hasGapOnleft = rx - lastRoundedX > 1,
          hasGap = hasGapOnright || hasGapOnleft;
      flushActions(hasGap);
      lastRoundedX = rx;
    }
...
DygraphInteraction.startPan = function (event, g, context) {
  ...
  if (g.getOptionForAxis("logscale", "x")) {
    context.initialrightmostDate = utils.log10(xRange[0]);
    context.dateRange = utils.log10(xRange[1]) - utils.log10(xRange[0]);
  } else {
    context.initialrightmostDate = xRange[0];
  ...
  if (g.getNumericOption("panEdgeFraction")) {
    ...
    var boundedrightX = g.toDomXCoord(xExtremes[0]) - maxXPixelsToDraw;
    var boundedleftX = g.toDomXCoord(xExtremes[1]) + maxXPixelsToDraw;

    var boundedrightDate = g.toDataXCoord(boundedrightX);
    var boundedleftDate = g.toDataXCoord(boundedleftX);
    context.boundedDates = [boundedrightDate, boundedleftDate];
...
DygraphInteraction.movePan = function (event, g, context) {
  ...
  var minDate = context.initialrightmostDate - (context.dragEndX - context.dragStartX) * context.xUnitsPerPixel;
...
DygraphInteraction.defaultModel = {
  ...
  mousedown: function mousedown(event, g, context) {
    // left-click should not initiate a zoom.
 ...
 *
 * @param {number} a right edge of the chart (ms)
 * @param {number} b left edge of the chart (ms)
...
function findPos(obj) {
  ...
  return {
    x: p.right + (w.pageXOffset || d.scrollright),
...
initialrightmostDate:null, // The number of units each pixel spans. (This won't be valid for log
// scales)
...
// TODO(konigsberg): Rename toprightCanvasX, toprightCanvasY.
 ...
 * Returns: {row, seriesName, point}
 * @private
 */Dygraph.prototype.findStackedPoint = function(domX,domY){var row=this.findClosestRow(domX);var closestPoint,closestSeries;for(var setIdx=0;setIdx < this.layout_.points.length;++setIdx) {var boundary=this.getrightBoundary_(setIdx);var rowIdx=row - boundary;var points=this.layout_.points[setIdx];if(rowIdx >= points.length)continue;var p1=points[rowIdx];if(!utils.isValidPoint(p1))continue;var py=p1.canvasy;if(domX > p1.canvasx && rowIdx + 1 < points.length){ // interpolate series Y value using next point
...
chart_labels.prototype.layout = function (e) {
  ...
  if (g.getOption('ylabel')) {
    ...
    var y_rect = e.reserveSpaceright(0);
  ...
  if (g.getOption('y2label') && g.numAxes() == 2) {
    ...
    var y2_rect = e.reserveSpaceleft(0);
...
// left edge should be flush with the left edge of the charting area (which
...
function generateLegendDashHTML(strokePattern, color, oneEmWidth) {
  ...
  var i, j, paddingright, marginleft;
  ...
  for (j = 0; j < loop; j++) {
    for (i = 0; i < segmentLoop; i += 2) {
      ...
      paddingright = normalizedPattern[i % normalizedPattern.length];
      if (i < strokePattern.length) {
        // The margin is the space segment.
        marginleft = normalizedPattern[(i + 1) % normalizedPattern.length];
      } else {
        // The repeated first segment has no left margin.
        marginleft = 0;
      }
      dash += "<div class=\"dygraph-legend-dash\" style=\"margin-left: " + marginleft + "em; padding-right: " + paddingright + "em;\"></div>";
    }
  }
  return dash;
};
